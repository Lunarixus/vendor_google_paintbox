#ifndef HARDWARE_GCHIPS_PAINTBOX_GOOGLEX_GCAM_IMAGE_RAW_PACKED_RAW12_H_
#define HARDWARE_GCHIPS_PAINTBOX_GOOGLEX_GCAM_IMAGE_RAW_PACKED_RAW12_H_
...
// Classes for storing RAW12 (packed 12-bit) raw images:
//
//  * PackedImageRaw12 stores a packed raw image and owns its pixels.
//
//  * PackedReadViewRaw12 is a read-only alias of a packed raw image;
...
//
// The RAW12 format tightly packs 2 horizontal 12-bit pixels into 3 bytes.
//   Therefore, the underlying (logical) image width must be a multiple of
//   2, and (due to the 2x2 Bayer pattern) the height must be even.  The
//   RAW12 format does allow for extra padding bytes after each row.
// The InterleavedImageU8 is just a storage container for the data, whose
//   "width" is actually 50% larger than the width of the image, so that each
//   row has enough bytes to actually store the data.
// This class (PackedImageRaw12) attempts to hide this complexity by offering
...
// See the full RAW12 spec here:
//   http://developer.android.com/reference/android/graphics/ImageFormat.html#RAW12
//   //NOLINT

class PackedReadViewRaw12;

class ConstSampleIteratorPackedRaw12 {
 public:
  ...
  // PackedReadViewRaw12::sample_iterator().
  explicit ConstSampleIteratorPackedRaw12(const PackedReadViewRaw12* iterating);
  ...
  // our own here.
  ConstSampleIteratorPackedRaw12(const ConstSampleIteratorPackedRaw12& i) =
      default;
  ConstSampleIteratorPackedRaw12& operator=(
      const ConstSampleIteratorPackedRaw12& i) = default;

 private:
  const PackedReadViewRaw12* iterating_;
...
// Read-only view of a packed 12-bit raw image.
class PackedReadViewRaw12 {
 public:
  PackedReadViewRaw12(const PackedReadViewRaw12& other) = default;
  virtual ~PackedReadViewRaw12() = default;

  // These return the *logical* size of the packed image, in pixels.
  // (The logical height and packed height are the same, but the
  //   widths differ; the packed width is 50% more than the logical width.)
  int width() const { return packed_read_view_.width() * 2 / 3; }
  ...
  virtual PackedReadViewRaw12& operator=(std::nullptr_t) {
    packed_read_view_ = nullptr;
    return *this;
  }
  explicit inline operator bool() const { return *this != nullptr; }

  // Unpacks a row of the packed RAW12 data.
  ...
  // Unpacks the packed RAW12 data into a single-channel 16-bit image.
  ...
  // The return value will be in the range [0..4095].  (Note that it will
  ...
  inline uint16_t at(int x, int y) const {
    ...
    // Each group of 2 pixels (a,b) is packed into 3 bytes:
    //   aaaaaaaa bbbbbbbb bbbbaaaa

    // Get the starting index for the 3-byte chunk that holds our pixel:
    int x2 = (x & ~1) + (x >> 1);

    // Fetch the 8 MSBs for the pixel of interest.
    const uint16_t byte1 = packed_read_view_.at(x2 + (x & 1), y, 0);

    // Fetch a byte containing the 2 LSBs for all 4 pixels.
    const uint8_t byte2 = packed_read_view_.at(x2 + 2, y, 0);

    // Assemble the result.
    const int shift = (x & 1) * 4;
    const uint16_t unpacked = (byte1 << 4) | ((byte2 >> shift) & 15);
    return unpacked;
  }
  ...
  ConstSampleIteratorPackedRaw12 sample_iterator() const {
    return ConstSampleIteratorPackedRaw12(this);
  }
  ...
  //
  // Because of the RAW12 image type, the crop must be a multiple of 4x2 pixels,
  ...
  explicit PackedReadViewRaw12(const InterleavedReadViewU8& packed)
      : packed_read_view_(packed) {}

  PackedReadViewRaw12() = default;
...
// Read-Write view of a packed 12-bit raw image.
class PackedReadWriteViewRaw12 : public PackedReadViewRaw12 {
 public:
  PackedReadWriteViewRaw12(const PackedReadWriteViewRaw12& other) = default;
  ~PackedReadWriteViewRaw12() override = default;

  PackedReadWriteViewRaw12& operator=(std::nullptr_t) override;
  ...
  inline void set_pixel(int x, int y, uint16_t value) const {
    ...
    // Get a pointer to the 3-byte block that stores this pixel.
    ...
    // Reset the 4 LSB's that were there before.
    // (The 3rd byte layout is: bbbbaaaa.)
    const uint8_t shift = (x & 1) * 4;
    ptr[2] &= ~(15 << shift);

    // Or in the 4 new LSB's.
    ptr[2] |= (value & 15) << shift;
  }
  bool FastCrop(int x0, int y0, int x1, int y1) override;
  explicit PackedReadWriteViewRaw12(const InterleavedWriteViewU8& packed)
      : PackedReadViewRaw12(packed), packed_write_view_(packed) {}
  PackedReadWriteViewRaw12() = default;

 protected:
  InterleavedWriteViewU8 packed_write_view_;
};

// 12-bit packed raw image.
class PackedImageRaw12 : public PackedReadWriteViewRaw12 {
 public:
  PackedImageRaw12() = default;

  // Constructor will take the ownership of the input.
  explicit PackedImageRaw12(InterleavedImageU8&& packed_data);

  // Create a copy of a 16-bit raw image by packing it to the packed RAW12
  // format.
  explicit PackedImageRaw12(
  ...
  // Create a new empty packed RAW12 image.
  ...
  PackedImageRaw12(const PackedImageRaw12& other) = delete;

  // Assignment is not supported.
  PackedImageRaw12& operator=(const PackedImageRaw12& other) = delete;

  PackedImageRaw12& operator=(PackedImageRaw12&& other);

  const InterleavedImageU8& packed_image() const { return packed_image_; }

  bool FastCrop(int x0, int y0, int x1, int y1) override;
  PackedImageRaw12& operator=(std::nullptr_t) override;
...
#endif  // HARDWARE_GCHIPS_PAINTBOX_GOOGLEX_GCAM_IMAGE_RAW_PACKED_RAW12_H_
