#ifndef HARDWARE_GCHIPS_PAINTBOX_GOOGLEX_GCAM_IMAGE_METADATA_STATIC_METADATA_H_
#define HARDWARE_GCHIPS_PAINTBOX_GOOGLEX_GCAM_IMAGE_METADATA_STATIC_METADATA_H_
...
// Gets static metadata for the given device code. Returns false if the device
// code was not found in a baked-in database.
//
// This function is necessary only to support legacy bursts where StaticMetadata
// was not serialized to disk, and for non-standard calls to Gcam::Create() that
// specify an optional device code override.
bool GetStaticMetadataFromDeviceCode(const char* device_code,
                                     StaticMetadata* static_metadata);

}  // namespace gcam

#endif  // HARDWARE_GCHIPS_PAINTBOX_GOOGLEX_GCAM_IMAGE_METADATA_STATIC_METADATA_H_
