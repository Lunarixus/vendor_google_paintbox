...
protected:
    ...
    struct OutgoingDataXfer {
        ...
        // is a reply needed?
        bool need_reply;
        // is transfer done (both DMA done and reply received)?
        bool xfer_done;
...
public:
    ...
    * timeout_ms is the waiting timeout when this function is blocking.  This
    * parameter is currently not used in mockeaselcomm's implementation.
    *
    * Returns 0 for success, -1 for failure.  Failure generally means Easel
    * is unresponsive or in an invalid state, and Easel is being reset.
    */
    int open(int service_id,
             __unused long timeout_ms = DEFAULT_OPEN_TIMEOUT_MS) override;
