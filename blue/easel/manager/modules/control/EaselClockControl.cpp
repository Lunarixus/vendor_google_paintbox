...
static enum EaselClockControl::Mode mMode =
    (enum EaselClockControl::Mode)-EINVAL;
...
static const std::unordered_map<enum EaselThermalMonitor::Condition,
                                struct ModeConfig> functionalModeConfigs = {
...
int EaselClockControl::setMode(enum Mode mode,
  ...
  if ((mode == mMode) && (mThermalCondition == thermalCond)) { return 0; }
  ...
  switch (mode) {
    ...
    case Mode::Functional:
    {
      ...
      ALOGI("%s: Functional Mode (%d/%d/%d)", __FUNCTION__, cfg.lpddrFreq,
            cfg.cpuFreq, cfg.ipuFreq);
      setIpuClockGating(false);
      setAxiClockGating(false);
...
bool EaselClockControl::isNewThermalCondition(
    enum EaselThermalMonitor::Condition thermalCond) {
...
int EaselClockControl::getLpddrFrequency() {
  ...
  int ret = readSysFile((char*)LPDDR_SYS_FILE, buf, 32);
  if (ret) {
...
int EaselClockControl::getProcessorFrequency(char *sysFile) {
  ...
  int ret = readSysFile(sysFile, buf, 32);
  if (ret) {
    return ret;
  }
...
int EaselClockControl::setProcessorFrequency(char *sysFile,
                                             const int *validFrequencies,
                                             int validCnt,
                                             int freq) {
  int i;
  char buf[32];
  ...
  snprintf(buf, sizeof(buf), "%d", validFrequencies[i]);

  return writeSysFile(sysFile, buf, sizeof(buf));
}
...
int EaselClockControl::getSys200Mode(bool *enable) {
  ...
  int ret = readSysFile((char*)SYS200_SYS_FILE, buf, 32);
  if (ret) {
    return ret;
  }
...
int EaselClockControl::setSys200Mode() {
  ...
  int ret = writeSysFile((char*)SYS200_SYS_FILE, buf, 32);
  if (ret) {
    return ret;
  }
