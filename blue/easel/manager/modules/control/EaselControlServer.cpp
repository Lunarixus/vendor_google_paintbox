...
static const std::chrono::milliseconds kHeartbeatPeriod =
...
void setTimeFromMsg(uint64_t boottime, uint64_t realtime) {
  ...
  if (clock_gettime(CLOCK_BOOTTIME, &ts) == 0) {
    timesync_local_boottime = (uint64_t)ts.tv_sec * NSEC_PER_SEC + ts.tv_nsec;
  } else {
    assert(0);
...
void printWaferId() {
  ...
  uint32_t upper = (buf[4] << 24) | (buf[5] << 16) | (buf[6] << 8) | buf[7];
  uint32_t lower = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
...
int EaselControlServer::open(EaselService service_id) {
  int ret = initializeServer(service_id);
  if (ret) {
    return ret;
  }
...
int EaselControlServer::getApSynchronizedClockBoottime(int64_t *clockval) {
  ...
  uint64_t now_local_boottime = (uint64_t)ts.tv_sec * NSEC_PER_SEC + ts.tv_nsec;
...
EaselControlServer::ThermalCondition EaselControlServer::setClockMode(
    ClockMode mode) {
  enum EaselThermalMonitor::Condition condition =
    thermalMonitor.checkCondition();
...
bool EaselControlServer::isNewThermalCondition() {
  return EaselClockControl::isNewThermalCondition(
